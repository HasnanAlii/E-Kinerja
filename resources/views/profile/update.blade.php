            ...
            <div class="bg-white shadow-lg rounded-lg overflow-hidden">
                <form method="post" action="{{ route('profil.update') }}" enctype="multipart/form-data">
                    ...
                    <header class="p-6 border-b border-gray-200 flex items-start gap-3">
                        ...
                        <div>
                            <h2 class="text-lg font-bold text-blue-900">Informasi Profil</h2>
                            <p class="mt-1 text-sm text-gray-600">Perbarui data profil Anda sesuai kebutuhan.</p>
                        </div>
                    </header>
                    ...
                    <div class="p-6 space-y-10">
                            ...
                            <div class="flex items-center gap-6">
                                ...
                                <div>
                                    <label for="profile_photo"
                                           class="cursor-pointer rounded-md font-medium text-blue-600 hover:text-blue-700">
                                        Ganti Foto
                                ...
                                @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                                    <div class="mt-3 bg-yellow-50 border border-yellow-200 p-4 rounded-md">
                                        ...
                                        <button form="send-verification"
                                                class="underline text-sm text-yellow-600 hover:text-yellow-700">
                        ...
                        @if ($pegawai)
                            <x-profile.section title="Data Pegawai" desc="Informasi kepegawaian Anda.">
                                ...
                                <x-profile.input-row id="pegawai_status" label="Status Kepegawaian">
                                    <x-text-input disabled class="w-full bg-gray-100"
                                                  value="{{ $pegawai->status ?? '-' }}" />
                                </x-profile.input-row>
                                <x-profile.input-row id="atasan_status" label="Masa Kerja">
                                    <x-text-input disabled class="w-full bg-gray-100"
                                                  value="{{ $masaKerjaPegawai ?? '-' }}" />
                                </x-profile.input-row>
                        ...
                        @if ($atasan)
                            <x-profile.section title="Data Atasan" desc="Informasi jabatan Anda sebagai Atasan.">
                                ...
                                <x-profile.input-row id="atasan_bidang" label="Bidang">
                                    <x-text-input disabled class="w-full bg-gray-100"
                                ...
                                <x-profile.input-row id="golongan" label="Golongan">
                                ...
                                <x-profile.input-row id="atasan_status" label="Status Kepegawaian">
                                    <x-text-input disabled class="w-full bg-gray-100"
                                                  value="{{ $atasan->status ?? '-' }}" />
                                </x-profile.input-row>
